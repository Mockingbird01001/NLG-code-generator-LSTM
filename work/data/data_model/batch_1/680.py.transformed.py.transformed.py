...
class DocumentMetaclass(type):
    def __new__(cls, name, bases, attrs):
        ...
        attrs['_types'] = attrs['_subclasses']
        new_class = super_new(cls, name, bases, attrs)
        for base in document_bases:
            if _cls not in base._subclasses:
                base._subclasses += (_cls,)
            base._types = base._subclasses
...
class TopLevelDocumentMetaclass(DocumentMetaclass):
    ...
    def get_auto_id_names(cls, new_class):
        ...
        if id_name not in new_class._fields and                id_db_name not in (v.db_field for v in new_class._fields.values()):
            return id_name, id_db_name
        id_basename, id_db_basename, i = 'auto_id', '_auto_id', 0
        while id_name in new_class._fields or                id_db_name in (v.db_field for v in new_class._fields.values()):
