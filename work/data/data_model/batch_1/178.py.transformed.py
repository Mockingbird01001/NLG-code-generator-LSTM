...
def write_xml(xml_pwd, xml_d, same_key_l, pretty_print=False):
    with open(xml_pwd, 'w') as f:
        ...
        xml_str = etree.tostring(etree.fromstring(xml_str), pretty_print=True).decode('utf-8')            if pretty_print else xml_str
        f.write(xml_str)
        return xml_str
    ...
    def csv_to_xml(self, csv_pwd):
        ...
        for i, csv_result in enumerate(csv_l):
            ...
            for j, (label, score, y_min, x_min, y_max, x_max) in enumerate(tf_result_l):
                tf_obj = self.add_img_object(label, score, x_min, y_min, x_max, y_max)
