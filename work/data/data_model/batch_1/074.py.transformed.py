...
def train_and_evaluate(args):
    ...
    file_size = os.path.getsize(save_path) / (2 ** 20)
...
def train_and_evaluate_mp_ring(args):
    ...
    for idx in range(num_worker):
        ...
        process_list.extend([mp.Process(target=mp_train, args=(args, idx, queue_data, pipe_eva2)),
...
def mp_evaluate(args, pipe_eva1):
    ...
    for _ in range(num_worker):
        pipe_eva1.send('stop')
    time.sleep(2)
def mp_train(args, idx, queue_data, pipe_eva2):
