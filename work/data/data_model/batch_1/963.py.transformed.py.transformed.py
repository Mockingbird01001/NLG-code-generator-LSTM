...
def main():
 ...
 if case == 1:
		P = map(int, list(raw_input("Enter 8 bit plaintext (eg. 10100101) - ")))
  if len(P) != 8:
			print "Enter only 8 bit plaintext!"
   sys.exit(0)
  K = map(int, list(raw_input("Enter 10 bit key (eg. 1110000110) - ")))
  if len(K) != 10:
 ...
 if case == 2:
		C = map(int, list(raw_input("Enter 8 bit ciphertext (eg. 10100101) - ")))
  if len(C) != 8:
