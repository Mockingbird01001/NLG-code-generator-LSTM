...
class ReplayBuffer1:
    ...
    def append_buffer(self, state, other):
        ...
        if self.next_idx >= self.max_len:
            self.if_full = True
            self.next_idx = 0
    def append_buffer_per(self, state, other):
        self.buf_state[self.next_idx] = state
        self.buf_other[self.next_idx] = other
    ...
    def extend_buffer_per(self, state, other):
        size = len(other)
        next_idx = self.next_idx + size
    ...
    def empty_buffer_before_explore(self):
        ...
        self.now_len = 0
        self.if_full = False
    def print_state_norm(self, neg_avg=None, div_std=None):
        ...
        rd.shuffle(indices)
        indices = indices[:max_sample_size]
...
class BinarySearchTree:
    def __init__(self, memo_len):
        ...
        self.prob_ary = np.zeros((memo_len - 1) + memo_len)
        self.max_len = len(self.prob_ary)
        self.now_len = self.memo_len - 1
        self.indices = None
        self.depth = int(np.log2(self.max_len))
        self.per_alpha = 0.6
        self.per_beta = 0.4
    def update_id(self, data_id, prob=10):
    ...
    def get_leaf_id(self, v):
        parent_idx = 0
        while True:
            l_idx = 2 * parent_idx + 1
            r_idx = l_idx + 1
            if l_idx >= (len(self.prob_ary)):
                leaf_idx = parent_idx
                break
            else:
                if v <= self.prob_ary[l_idx]:
                    ...
                    parent_idx = r_idx
        return min(leaf_idx, self.now_len - 2)
    def get_indices_is_weights(self, batch_size, beg, end):
        ...
        is_weights = np.power(prob_ary, -self.per_beta)
