...
class DrawROI(object):
    ...
    def confirm(self, img):
        ...
        high_light_roi_mat = cv2.fillPoly(high_light_roi_mat, self.roi_pts, (1.0, 1.0, 1.0))
        canvas = np.copy(img)
        ...
        while os.path.exists(self.roi_pts_pwd):
            ...
            if wait_key == 13:
                ...
                os.remove(self.roi_pts_pwd)
                self.roi_pts = []
                self.done = False
    def draw_roi(self, line_color=(234, 234, 234)):
        img = self.img
        roi_mat = np.zeros(img.shape, dtype=np.uint8)
        ...
        while not self.done:
            ...
            if len(self.roi_pts) > 0:
                cv2.line(canvas_cur, self.roi_pts[-1], self.cur_pt, line_color, 2)
            cv2.imshow(self.window_name, canvas_cur)
...
class EdgeDetection(object):
    def __init__(self, img, roi_pts):
        ...
        background_change_after_read_image_number = 64
        ...
        self.high_light_roi_mat = cv2.fillPoly(np.ones(self.roi_mat.shape, dtype=np.float) * 0.25,
                                               self.roi_pts, (1.0, 1.0, 1.0), )
        self.img_len0 = int(360)
        self.img_len1 = int(self.img_len0 / (img.shape[0] / img.shape[1]))
        self.img_back = self.img_preprocessing(img)
        self.min_side_num = 3
        self.min_side_len = int(self.img_len0 / 24)
    ...
    def main_get_img_show(self, origin_img):
        ...
        self.img_back = img            if not contours or not self.get_polygon_contours(img, img_prev)            else self.img_back
