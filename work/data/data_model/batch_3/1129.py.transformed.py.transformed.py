...
def init_session():
    ...
    layer2 = tf.nn.dropout(layer1, keep_prob1)
    ...
    with tf.name_scope('Loss'):
        loss = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(logits=pred, labels=label))
...
def train_session(sess_para, data_para, save_gap):
    ...
    sum_writer = tf.summary.FileWriter(G.logs_dir, graph=tf.get_default_graph())
    '''train loop init'''
    time1 = time.time()
    accuracy = 0.0
